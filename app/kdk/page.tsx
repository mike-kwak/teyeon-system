...
export default function KDKPage() {
    ...
    const [toastMsg, setToastMsg] = useState("Í≤∞Í≥ºÍ∞Ä ?àÏ†Ñ?òÍ≤å Í∏∞Î°ù?òÏóà?µÎãà??);

    // [v12.0] Í∞úÎ∞©??Í∂åÌïú ?úÏä§?? Admin ?¨Î? ?êÎ≥Ñ (ADMIN ?¨Ìï®)
    const isAdmin = role === 'CEO' || role === 'ADMIN';
    const clubId = process.env.NEXT_PUBLIC_CLUB_ID || "512d047d-a076-4080-97e5-6bb5a2c07819";

    // Í∂åÌïú ?úÌïú ?åÎ¶º ?¨Ìçº
    const triggerAccessDenied = (msg: string = "Í¥ÄÎ¶¨ÏûêÎß??ÄÏßÑÏùÑ ?ùÏÑ±/?òÏ†ï?????àÏäµ?àÎã§.") => {
        setToastMsg(msg);
        setShowToast(true);
    ...
    useEffect(() => {
        ...
        console.clear();
        console.log("?èÅ TEYEON SYSTEM v5.1 STABLE: ALL SYSTEMS GO");
    }, []);
    ...
    useEffect(() => {
        if (role === 'GUEST') {
            alert("?ïÌöå???¥ÏÉÅÎß??¥Ïö© Í∞Ä?•Ìïú Î©îÎâ¥?ÖÎãà?? ?Ä?úÎ≥¥?úÎ°ú ?¥Îèô?©Îãà??");
            router.push('/');
        }
    }, [role, router]);
    */
    ...
    const [totalCourts, setTotalCourts] = useState(99); // [v17.0] Î¨¥Ï†ú??ÏΩîÌä∏ ÏßÄ??(CEO ÏßÄ??
    ...
    const [matchRules, setMatchRules] = useState("1:1 ?úÏûë, ?∏Ïóê?? ?Ä??3:3 ?úÏûë 7?¨Ïù∏???†Ïäπ");
    ...
    const [accountInfo, setAccountInfo] = useState("Ïπ¥Ïπ¥?§Î±Ö??3333-01-5235337 (Í≥ΩÎ???");
    ...
    // [v14.0] ?úÎ™© Ï§ëÎ≥µ Î∞©Ï?: ?êÎèô?ºÎ°ú ?§Ïùå Î≤àÌò∏(KDK_02 ??Î•?Ï∞æÎäî ?¨Ìçº
    const getDisplayUrl = () => {
    ...
    const openDisplayBoard = () => {
        ...
        if (!displayUrl) {
            alert("Î®ºÏ? KDK ?∏ÏÖò???ùÏÑ±?òÍ±∞??Î∂àÎü¨?Ä Ï£ºÏÑ∏??");
            return;
        }
    ...
    const copyDisplayBoardUrl = async () => {
        ...
        try {
            ...
            alert("?ÑÍ¥ë??Ï£ºÏÜåÍ∞Ä Î≥µÏÇ¨?òÏóà?µÎãà??");
        } catch {
            alert(`?ÑÍ¥ë??Ï£ºÏÜå: ${absoluteUrl}`);
        }
    };
    ...
    const findNextAvailableTitle = async () => {
        ...
        try {
            ...
            // 2. ?úÎ≤Ñ ?ÑÏπ¥?¥Î∏å ?ïÏù∏ (ÏµúÍ∑º 20Í∞úÎßå) - ?åÏù¥Î∏îÏù¥ ?ÜÏùÑ ???àÏúºÎØÄÎ°??àÏ†Ñ?òÍ≤å Ï≤òÎ¶¨
    ...
    // --- [v7.0 ABSOLUTE] 12??13Í∏? ?úÎ≤Ñ Íµ¥Î≥µ Î∞?Î°úÏª¨ Í∞ïÏ†ú ?Ä???µÌï© ?¨Ìçº ---
    const absoluteSyncRPC = async (data: any) => {
        const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || '';
        const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || '';
        
        // 1. ?ÑÏû• DB ID Í≤ΩÎ≥¥ (CEO Verification)
        const projectId = supabaseUrl.split('//')[1]?.split('.')[0] || 'Unknown';
        console.log(`?èôÔ∏?[ABSOLUTE] TARGET PROJECT ID: ${projectId}`);
        console.table(data);
        ...
        try {
            ...
            const response = await fetch(endpoint, {
                ...
                headers: {
                    ...
                    'Prefer': 'params=single-object', // ÎßàÎ≤ï???§Îçî: JSON ?ÑÏ≤¥Î•??®Ïùº ?∏ÏûêÎ°?Îß§Ìïë
                    'x-client-info': 'teyeon-absolute-v7.0'
                },
                body: JSON.stringify(data) // ?òÌïë ?ÜÏù¥ ?úÏàò Í∞ùÏ≤¥ ?ÑÏÜ°
            });
            ...
            // 3. '12??13Í∏? Î°úÏª¨ Í∞ïÏ†ú ?Ä??(Offline First)
            try {
                ...
                console.warn("?õ°Ô∏?[ABSOLUTE] DATA SECURED IN LOCAL STORAGE");
                return { error: null }; // ?±Í≥µ?ºÎ°ú Í∞ÑÏ£º?òÏó¨ ?îÎ©¥ ÏßÑÌñâ
            } catch (localErr) {
    ...
    const handleFinalArchive = async () => {
        if (!confirm("?èÜ ?Ä?åÎ? Í≥µÏãù?ÅÏúºÎ°?Ï¢ÖÎ£å?òÍ≥† '?¨Ï∏µ Í∏∞Î°ù????Î∞ïÏ†ú?òÏãúÍ≤†Ïäµ?àÍπå?\n(?ºÏù¥Î∏??∞Ïù¥?∞Í? ??†ú?òÍ≥† ?ÑÏπ¥?¥Î∏å ?¨ÌÑ∏Î°?Ï¶âÏãú ?¥Îèô?©Îãà??)")) return;

        try {
            // [CEO ?ÑÏû• Í≤ÄÏ¶?Î°úÏßÅ Í∞Ä??
            ...
            // [v10.0 SAVE OR DIE] ?µÏã† ??Î°úÏª¨??Ï¶âÏãú Î∞ïÏ†ú (Safety First)
            try {
                const failovers = JSON.parse(localStorage.getItem('kdk_archive_failover') || '[]');
                const failoverItem = {
                    id: sessionId,
                    raw_data: rawDataPayload,
                    failover: true, // ?úÎ≤Ñ ?ïÏù∏ ?ÑÍπåÏßÄ???ÑÏãú ?ÅÌÉú
                ...
                console.warn("?õ°Ô∏?[v10.0] PRE-EMPTIVE LOCAL BACKUP SECURED");
            } catch (err) {
            ...
            try {
                ...
                alert(`?úÎ≤Ñ ?µÏã† ÏßÄ?∞ÏúºÎ°?Í∏∞Í∏∞???ÑÏãú ?Ä?•Îêò?àÏäµ?àÎã§. (${err.message})\n?ÑÏπ¥?¥Î∏å?êÏÑú ?ïÏù∏ Í∞Ä?•Ìï©?àÎã§.`);
            }

            // [v10.0] Championship Celebration (Í∏àÏÉâ Í∞ÄÎ£?ÎøåÎ¶¨Í∏?
            ...
            alert("Í≥µÏãù Ï¢ÖÎ£å ?§Ìå®: " + err.message);
        } finally {
    ...
    const validateGroups = (): { ok: boolean; msg: string } => {
        ...
        if (countA > 0 && countA < 4) {
            return { ok: false, msg: "AÏ°??∏Ïõê??Î∂ÄÏ°±Ìï©?àÎã§ (ÏµúÏÜå 4Î™??ÑÏöî)" };
        }
        if (countB > 0 && countB < 4) {
            return { ok: false, msg: "BÏ°??∏Ïõê??Î∂ÄÏ°±Ìï©?àÎã§ (ÏµúÏÜå 4Î™??ÑÏöî)" };
        }
        if (countA === 0 && countB === 0) {
            return { ok: false, msg: "ÏµúÏÜå 1Î™??¥ÏÉÅ??Ï∞∏ÏÑù?êÍ? ?ÑÏöî?©Îãà?? };
        }
    ...
    useEffect(() => {
        if (!hasInitializedKdkRef.current) {
            ...
            // [v14.0] Ï¥àÍ∏∞ Íµ¨Îèô ???úÎ™© Ï§ëÎ≥µ Ï≤¥ÌÅ¨
            setTimeout(() => {
    ...
    const normalizeStoredKdkGroup = (value?: string) => {
        ...
        if (raw.includes('BLUE') || raw === 'B' || raw.startsWith('BÏ°?.toUpperCase())) return 'B';
        if (raw.includes('GOLD') || raw === 'A' || raw.startsWith('AÏ°?.toUpperCase())) return 'A';
    ...
    const startNewKdkCreation = () => {
        if (!isAdmin) {
            triggerAccessDenied("?àÎ°ú??Í≤ΩÍ∏∞??Í¥ÄÎ¶¨ÏûêÎß??ùÏÑ± Í∞Ä?•Ìï©?àÎã§.");
            return;
        }
    ...
    const handleDeleteActiveSession = async () => {
        ...
        if (!isAdmin) {
            triggerAccessDenied("ÏßÑÌñâ Ï§ëÏù∏ ?∏ÏÖò ??†ú??Í¥ÄÎ¶¨ÏûêÎß?Í∞Ä?•Ìï©?àÎã§.");
        ...
        try {
            ...
            alert("ÏßÑÌñâ Ï§ëÏù∏ ?∏ÏÖò ??†ú???§Ìå®?àÏäµ?àÎã§. ?†Ïãú ???§Ïãú ?úÎèÑ??Ï£ºÏÑ∏??");
        } finally {
    ...
    const toggleMember = (id: string) => {
        ...
        if (next.has(id)) {
            ...
            if (member && !attendeeConfigs[id]) {
                const isAwardWinner = (member.achievements || '').includes('?∞Ïäπ') ||
                    (member.achievements || '').includes('Ï§Ä?∞Ïäπ') ||
                    (member.achievements || '').includes('?ÖÏÉÅ');
    ...
    const getPlayerName = (id: string, matchId?: string) => {
        ...
        if (!name) {
            return isLikelyPlayerId(id) ? "?¥Î¶Ñ ?ïÏù∏Ï§? : id;
        }
    ...
    const generateKDK = async () => {
        ...
        if (countA < 4) {
            setWarningMsg("AÏ°??∏Ïõê??Î∂ÄÏ°±Ìï©?àÎã§ (ÏµúÏÜå 4Î™??ÑÏöî)");
            setShowWarning(true);
            return;
        }

        if (countB > 0 && countB < 4) {
            setWarningMsg("BÏ°??∏Ïõê??Î∂ÄÏ°±Ìï©?àÎã§ (ÏµúÏÜå 4Î™??ÑÏöî)");
            setShowWarning(true);
            return;
        }
        ...
        try {
            ...
            if (fixedPartners.length > 0 && !fixedTeamMode) {
                ...
                if (unfulfilledPairs.length > 0) {
                    setTimeout(() => {
                        setWarningMsg("Í≥†Ï†ï ?åÌä∏?àÍ? ?àÎ¨¥ ÎßéÏïÑ 100% Îß§Ïπ≠??Î∂àÍ??•Ìï©?àÎã§.\\n?ºÎ? ?ÄÏßÑÏ? ?òÎèô?ºÎ°ú Ï°∞Ï†ï??Ï£ºÏÑ∏??");
                        setShowWarning(true);
                    }, 500);
                }
            }
            ...
            try {
                ...
                if (fullError) {
                    ...
                    if (isSchemaError) {
                        ...
                        console.warn("?†Ô∏è Full sync restricted. Missing columns or stale cache detector on:", fullError.message);
                        ...
                        console.log("?πÔ∏è Legacy Fallback Sync Active");
                    } else {
                        throw fullError;
                    }
                } else {
    ...
    const saveManualKDKMatches = async () => {
        ...
        if (!title) {
            alert('?∏ÏÖòÎ™ÖÏùÑ ?ÖÎ†•??Ï£ºÏÑ∏??');
            return;
        }

        if (manualPastePreview.length === 0) {
            alert('?Ä?•Ìï† ?òÎèô ?ÄÏßÑÏù¥ ?ÜÏäµ?àÎã§.');
            return;
        }

        const invalidRow = manualPastePreview.find(row => !row.isValid);
        if (invalidRow) {
            alert(`?ïÏãù???ïÏù∏??Ï£ºÏÑ∏?? ${invalidRow.raw}`);
            return;
        }
        ...
        try {
            ...
            const formattedMatches = manualPastePreview.map((row, index) => {
                ...
                if (uniquePlayers.size !== 4) {
                    throw new Error(`${getManualGroupLabel(group)} ${row.order}Î≤?Í≤ΩÍ∏∞ ?àÏóê Í∞ôÏ? ?†ÏàòÍ∞Ä Ï§ëÎ≥µ?òÏñ¥ ?àÏäµ?àÎã§.`);
                }

                const signature = `${group}|${playerIds.join('|')}|${row.time || ''}`;
                if (seenMatchSignatures.has(signature)) {
                    throw new Error(`${getManualGroupLabel(group)} ${row.order}Î≤?Í≤ΩÍ∏∞?Ä Í∞ôÏ? ?ÄÏßÑÏù¥ Ï§ëÎ≥µ?òÏñ¥ ?àÏäµ?àÎã§.`);
                }
    ...
    const startMatch = async (matchId: string) => {
        ...
        if (targetMatch.status === 'playing') {
            setToastMsg("?¥Î? ?úÏÑ±?îÎêú Í≤ΩÍ∏∞?ÖÎãà??);
            setShowToast(true);
            setTimeout(() => setShowToast(false), 3000);
            return;
        }

        // 1. Court Limit Check REMOVED (CEO: "ÏΩîÌä∏ ?òÎäî Î¨¥Ï†ú?úÏù¥???ùÍ∞Å?òÎ©¥ ??)
        ...
        if (conflictPlayers.length > 0) {
            ...
            setToastMsg(`Ï∞∏Ïó¨??Ï§ëÎ≥µ: ${names} ?†ÏàòÍ∞Ä ?¥Î? Í≤ΩÍ∏∞ Ï§ëÏûÖ?àÎã§`);
            setShowToast(true);
            if (window.navigator?.vibrate) window.navigator.vibrate([100, 50, 100]);
            setTimeout(() => setShowToast(false), 3000);
            return;
        ...
        try {
            ...
            // DB Sync via Native Update (RPC ?∞Ìöå)
    ...
    const execFullRepair = async () => {
        ...
        try {
            ...
            setSyncStatus('HEALTHY');
            setSyncErrorMsg(null);
            setToastMsg("?∞Ïù¥??Íµ¨Ï°∞Í∞Ä ?±Í≥µ?ÅÏúºÎ°?Î≥µÍµ¨?òÏóà?µÎãà??");
            setShowToast(true);
            setTimeout(() => setShowToast(false), 3000);
            ...
            alert(`?ö® Î≥µÍµ¨ ?§Ìå®: ${err.message}\n\n[Ï¢ÖÌï© ?†Î¨º ?∏Ìä∏ SQL]??Î®ºÏ? ?§Ìñâ?àÎäîÏßÄ ?ïÏù∏??Ï£ºÏÑ∏??`);
        } finally {
            setIsGenerating(false);
        }
    };
    ...
    const cancelMatch = async (matchId: string) => {
        try {
            ...
            // 1. Supabase Sync via Native Update (RPC ?∞Ìöå)
            ...
            alert("Í≤ΩÍ∏∞ Ï∑®ÏÜå Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§: " + err.message);
        }
    };

    const updateMatchCourt = (matchId: string) => {
        const courtStr = prompt("Î≥ÄÍ≤ΩÌï† ÏΩîÌä∏ Î≤àÌò∏Î•??ÖÎ†•?òÏÑ∏??, "1");
    ...
    const finishMatch = async (matchId: string, s1: number, s2: number) => {
        try {
            ...
            // 3. DB Sync via Native Update (RPC ?∞Ìöå)
            const { error: syncError } = await supabase
                .from('matches')
            ...
            alert("Í≤ΩÍ∏∞ Í≤∞Í≥º ?Ä?•Ïóê ?§Ìå®?àÏäµ?àÎã§: " + (err.message || "Unknown error"));
    ...
    const playerDetailedResults = useMemo<PlayerDetailedResult[]>(() => {
        ...
        const ensureDetail = (playerId: string, fallbackName?: string, fallbackGroup?: string) => {
            ...
            const name = resolvedName && resolvedName !== '?¥Î¶Ñ ?ïÏù∏Ï§?
                ? resolvedName
                : (fallbackName || rankingPlayer?.name || '?¥Î¶Ñ ?ïÏù∏Ï§?);
        ...
        allPlayersInRanking.forEach((player, index) => {
            ...
            const name = resolvedName && resolvedName !== '?¥Î¶Ñ ?ïÏù∏Ï§? ? resolvedName : player.name;
    ...
    const execCopySchedule = () => {
        if (!matches || matches.length === 0) {
            alert("?∞Ïù¥?∞Î? Î∂àÎü¨?§Îäî Ï§ëÏûÖ?àÎã§...");
            return;
        }

        // [v3.2 - FINAL REQUESTED TEMPLATE]
        let text = `?ìå ?§Îäò???ÄÏßÑÌëú: ${sessionTitle || 'Live Tournament'}\n`;
        text += `?ñÔ∏è Í∑úÏπô: ${matchRules || '1:1 ?úÏûë, ?∏Ïóê?? ?Ä??3:3 ?úÏûë 7?¨Ïù∏???†Ïäπ'}\n`;
        text += `?í∞ ?ÅÎ≤åÍ∏? ?∞Ïäπ ${firstPrize.toLocaleString()} / Î≤åÍ∏à ${bottom25Late.toLocaleString()} / Î≤åÍ∏à ${bottom25Penalty.toLocaleString()}\n`;
        text += `?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ\n`;
        ...
        uniqueGroups.forEach(group => {
            ...
            if (hasMultipleGroups) {
                text += `\n?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ\n`;
            }
        });
        ...
        text += `\n???ÅÏÑ∏ Í≤∞Í≥º ?ïÏù∏: https://teyeon-system.vercel.app/kdk`;

        console.log("Copied Match Schedule v3.2 (No Courts, No 'ÏßÄÍ∞?)");
        navigator.clipboard.writeText(text);
        alert("?§ÏãúÍ∞??ÄÏßÑÌëúÍ∞Ä ?¥Î¶ΩÎ≥¥Îìú??Î≥µÏÇ¨?òÏóà?µÎãà?? ??);
    };
    ...
    const copyFinalResults = () => {
        ...
        let text = `?èÜ ?§Îäò??ÏµúÏ¢Ö Í≤∞Í≥º: ${sessionTitle || 'Live Tournament'}\n`;
        text += `?è¶ Í≥ÑÏ¢å: ${accountInfo}\n`;
        text += `?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ\n\n`;
        const formatFinalResultName = (player: any) => {
            ...
            if (!cleanName || isLikelyPlayerId(cleanName)) return "πÃ»Æ¿Œ";
        ...
        sortedPlayers.forEach((p, i) => {
            ...
            let rankPrefix = (originalRank === 1) ? '?•á ' : (originalRank === 2) ? '?•à ' : (originalRank === 3) ? '?•â ' : `${originalRank}??`;
            ...
            if (originalRank === 1 && !p.is_guest) {
                prizePenaltyText = ` [?í∞ +${firstPrize.toLocaleString()}??`;
            } else if (isPenaltyTier) {
                prizePenaltyText = ` [?í∏ -${bottom25Penalty.toLocaleString()}??`;
            } else if (isFineTier) {
                prizePenaltyText = ` [?í∏ -${bottom25Late.toLocaleString()}??`;
            } else {
                prizePenaltyText = ` [0??`;
            }
        ...
        text += `\n?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ?Å‚îÅ\n`;
        text += `???ÑÏ≤¥ ?ÑÏπ¥?¥Î∏å ?ïÏù∏: https://teyeon-system.vercel.app/archive?session=${sessionId}`;

        navigator.clipboard.writeText(text);
        alert("ÏµúÏ¢Ö Í≤∞Í≥º Î∞??ïÏÇ∞ ?ÑÌô©??Î≥µÏÇ¨?òÏóà?µÎãà?? ??);
    };
    ...
    const normalizeManualGroup = (value?: string) => {
        const normalized = (value || 'A').trim().replace(/Ï°?/i, '').toUpperCase();
        return /^[A-Z]$/.test(normalized) ? normalized : 'A';
    };

    const getManualGroupLabel = (group?: string) => `${normalizeManualGroup(group)}Ï°?;

    const getManualGroupFromToken = (token?: string) => {
        if (!token) return "";
        const trimmed = token.trim();
        const match = trimmed.match(/^([A-Za-z])\s*Ï°?$/i);
    ...
    const parseManualPasteMatches = (input: string): ManualPastePreviewMatch[] => {
        ...
        input
            ...
            .forEach(line => {
                ...
                const groupPrefixMatch = withoutTime.match(/^([A-Za-z])\s*Ï°?\s+(?=\d)/i);
                if (groupPrefixMatch) {
    ...
    const getManualResolvedPlayer = (name: string) => {
        ...
        return {
            ...
            name: isGuest ? (trimmed ? `${trimmed}(G)` : "?¥Î¶Ñ ?ïÏù∏Ï§?) : memberName,
            rawName: trimmed,
            isGuest,
        };
    };
        ...
        <main className="flex min-h-screen w-full flex-col bg-black text-white font-sans relative overflow-hidden">
            ...
            <header className="relative z-10 p-8 flex flex-col items-center text-center">
                ...
                <h1 className="text-4xl font-black italic tracking-tighter text-white uppercase drop-shadow-2xl">
                    ÏßÑÌñâ Ï§ëÏù∏ ?∏ÏÖò ?†ÌÉù
                </h1>
                <p className="mt-3 max-w-sm text-[12px] font-bold leading-relaxed text-white/45">
                    Supabase???Ä?•Îêú ÏßÑÌñâ Ï§??∏ÏÖò Í∏∞Ï??ºÎ°ú ?¥ÏòÅ???ÄÏßÑÏùÑ ?†ÌÉù?©Îãà??
                </p>
                ...
                {allActiveSessions.length === 0 ? (
                    <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
                        <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-full border border-dashed border-white/20 bg-white/[0.03]">
                            <span className="text-4xl">?éæ</span>
                        </div>
                        <h2 className="mb-2 text-xl font-black uppercase tracking-tight text-white">ÏßÑÌñâ Ï§ëÏù∏ ?ÄÏßÑÏù¥ ?ÜÏäµ?àÎã§</h2>
                        <p className="max-w-xs text-[12px] font-bold leading-relaxed text-white/42">
                            ???ÄÏßÑÏ? Î©îÏù∏ ?îÎ©¥???ÄÏß??ùÏÑ±?êÏÑú ?úÏûë??Ï£ºÏÑ∏??
                        </p>
                        {isAdmin && (
                            <button
                                type="button"
                                ...
                                >
                                    ...
                                    <div className="grid grid-cols-3 gap-4 border-t border-white/5 pt-5">
                                        <div className="flex flex-col">
                                            <span className="mb-1 text-[8px] font-black uppercase tracking-widest text-white/20">Ï∞∏Í? ?∏Ïõê</span>
                                            <span className="text-lg font-black italic text-white">{s.playerCount}<span className="ml-0.5 text-[10px] opacity-30">Î™?/span></span>
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="mb-1 text-[8px] font-black uppercase tracking-widest text-white/20">Í≤ΩÍ∏∞ ??/span>
                                            <span className="text-lg font-black italic text-white">{s.matchCount}<span className="ml-0.5 text-[10px] opacity-30">Í∞?/span></span>
                                        </div>
                                        <div className="flex flex-col items-end justify-end">
                                            <span className="text-[10px] font-black uppercase tracking-tight text-[#C9B075]">?ÖÏû•?òÍ∏∞ ??/span>
                                        </div>
                                    </div>
                                </button>
            ...
            {sessionDeleteTarget && (
                <CustomConfirmModal
                    title="?∏ÏÖò ??†ú"
                    message={`${sessionDeleteTarget.title} ?∏ÏÖò??ÏßÑÌñâ Ï§?Í≤ΩÍ∏∞ ?∞Ïù¥?∞Î? ??†ú?©Îãà?? ?ÑÏπ¥?¥Î∏å ?∞Ïù¥?∞Îäî ??†ú?òÏ? ?äÏäµ?àÎã§.`}
                    confirmText={isDeletingSession ? "??†ú Ï§?.." : "?∏ÏÖò ??†ú"}
                    icon="?óëÔ∏?
            ...
            <main className="flex min-h-screen w-full flex-col bg-black px-6 py-8 text-white font-sans">
                <header className="mx-auto flex w-full max-w-lg flex-col items-center text-center">
                    ...
                    <h1 className="text-3xl font-black italic uppercase tracking-tight text-white">
                        ÏßÑÌñâ Ï§ëÏù∏ ?ÄÏßÑÏù¥ ?àÏäµ?àÎã§
                    </h1>
                    <p className="mt-3 max-w-sm text-[12px] font-bold leading-relaxed text-white/45">
                        Í∏∞Ï°¥ ?ÄÏßÑÏùÑ ?¥Ïñ¥???¥ÏòÅ?òÍ±∞?? ???ÄÏß??ùÏÑ± ?îÎ©¥?ºÎ°ú ?¥Îèô?????àÏäµ?àÎã§.
                    </p>
                </header>
                ...
                <section className="mx-auto mt-10 grid w-full max-w-lg gap-4">
                    ...
                    >
                        ...
                        <h2 className="text-2xl font-black tracking-tight text-white">Í∏∞Ï°¥ ?ºÏù¥Î∏?ÏΩîÌä∏ Î≥¥Í∏∞</h2>
                        <p className="mt-3 text-[12px] font-bold leading-relaxed text-white/50">
                            {sessionCount > 1 ? `${sessionCount}Í∞úÏùò ÏßÑÌñâ Ï§ëÏù∏ ?∏ÏÖò Ï§??†ÌÉù?©Îãà??` : `${primaryTitle || '?ÑÏû¨ ?∏ÏÖò'}???¥Ïñ¥???¥ÏòÅ?©Îãà??`}
                        </p>
                    </Link>
                    ...
                    >
                        ...
                        <h2 className="text-2xl font-black tracking-tight text-white">???ÄÏß??ùÏÑ±?òÍ∏∞</h2>
                        <p className="mt-3 text-[12px] font-bold leading-relaxed text-white/50">
                            Í∏∞Ï°¥ ?∏ÏÖò?Ä Î≥¥Ï°¥?òÍ≥†, ?êÎèô ?ùÏÑ± ?êÎäî ?òÎèô Íµ¨ÏÑ± Step 0Î∂Ä???àÎ°ú ?úÏûë?©Îãà??
                        </p>
                    </Link>
                </section>
            </main>
        ...
        return (
            <main className="flex min-h-screen w-full flex-col bg-black px-6 py-8 text-white font-sans">
                ...
                <header className="mx-auto flex w-full max-w-lg flex-col items-center text-center">
                    ...
                    <p className="mt-3 max-w-sm text-[12px] font-bold leading-relaxed text-white/45">
                        ?êÎèô ?ùÏÑ±Í≥??òÎèô Íµ¨ÏÑ±??Î®ºÏ? Î∂ÑÎ¶¨?¥ÏÑú ?àÏ†ï?ÅÏúºÎ°??ÄÏßÑÏùÑ Ï§ÄÎπÑÌï©?àÎã§.
                    </p>
                </header>
                ...
                <section className="mx-auto mt-10 grid w-full max-w-lg gap-4">
                    ...
                    >
                        ...
                        <h2 className="text-2xl font-black tracking-tight text-white">?êÎèô ?ùÏÑ±</h2>
                        <p className="mt-3 text-[13px] font-bold leading-relaxed text-white/50">
                            Ï∞∏Í??êÏ? Ï°∞Í±¥???†ÌÉù?òÎ©¥ ?±Ïù¥ Í∏∞Ï°¥ KDK Î°úÏßÅ?ºÎ°ú ?ÄÏßÑÏùÑ ?êÎèô ?ùÏÑ±?©Îãà??
                        </p>
                    </button>
                    ...
                    >
                        ...
                        <h2 className="text-2xl font-black tracking-tight text-white">?òÎèô Íµ¨ÏÑ±</h2>
                        <p className="mt-3 text-[13px] font-bold leading-relaxed text-white/50">
                            ÏßÅÏ†ë ÎßåÎì† ?ÄÏß? ?ëÏ?/?çÏä§??Î∂ôÏó¨?£Í∏∞, Ï∫°Ï≤ò ?∏Ïãù Î∞©Ïãù?ºÎ°ú ?ÄÏßÑÏùÑ Íµ¨ÏÑ±?©Îãà??
                        </p>
                    </button>
                </section>
            </main>
                        ...
                        <>
                            ...
                            >
                                <div className="mb-6 flex items-center justify-between">
                                    ...
                                    <span className="rounded-full bg-[#C9B075] px-3 py-1 text-[10px] font-black text-black">1Ï∞?/span>
                                </div>
                                <h2 className="text-xl font-black text-white">?ÄÏß?Î∂ôÏó¨?£Í∏∞</h2>
                                <p className="mt-2 text-[12px] font-bold leading-relaxed text-white/45">
                                    ?ëÏ?, Ïπ¥ÌÜ°, Î©îÎ™®???çÏä§?∏Î? Î∂ôÏó¨?£Ïñ¥ Í≤ΩÍ∏∞ ?úÏÑú?Ä ?Ä Íµ¨ÏÑ±??ÎØ∏Î¶¨ ?ïÏù∏?©Îãà??
                                </p>
                            </button>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                >
                                    ...
                                    <h3 className="mt-4 text-base font-black text-white">ÏßÅÏ†ë ÎßåÎì§Í∏?/h3>
                                    <p className="mt-2 text-[11px] font-bold leading-relaxed text-white/35">Í≤ΩÍ∏∞Î≥??Ä ÏßÅÏ†ë Íµ¨ÏÑ±</p>
                                </button>
                                <button
                                    type="button"
                                    onClick={() => {
                                ...
                                >
                                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30">Coming Soon</span>
                                    <h3 className="mt-4 text-base font-black text-white">Ï∫°Ï≤ò ?∏Ïãù</h3>
                                    <p className="mt-2 text-[11px] font-bold leading-relaxed text-white/35">?¥Î?ÏßÄ/OCR Í∏∞Î∞ò ?∏Ïãù</p>
                                </button>
                            </div>
                        </>
                    )}
                        ...
                        <div className="space-y-4">
                            <div className="overflow-visible rounded-[24px] border border-[#C9B075]/20 bg-[#141414] p-5">
                                <div className="mb-4">
                                    ...
                                    <h2 className="mt-1 text-xl font-black text-white">?ÄÏß?Î∂ôÏó¨?£Í∏∞</h2>
                                </div>
                                <div className="mb-4 rounded-[16px] border border-[#C9B075]/20 bg-black/30 p-3">
                                    <div className="mb-2 flex items-center justify-between gap-2">
                                        <span className="text-[10px] font-black uppercase tracking-[0.18em] text-[#C9B075]">?ÖÎ†• ?àÏãú</span>
                                        <span className="text-[9px] font-bold text-white/35">??Íµ¨Î∂Ñ ?çÏä§??ÏßÄ??/span>
                                    </div>
                                     <pre className="whitespace-pre-wrap break-keep text-[11px] font-bold leading-relaxed text-white/55">
{`AÏ°?
1 Î¥âÏ?/?ÅÏú§ vs ?ÅÌò∏/Í¥ëÌòÑ 19:00
BÏ°?
1 ÎØºÏ?/?ÅÏ? vs Í∞ïÏ†ï??Íµ¨Î¥âÏ§Ä 20:00
A    1    Î¥âÏ?    ?ÅÏú§    ?ÅÌò∏    Í¥ëÌòÑ    19:00`}
                                     </pre>
                                    <p className="mt-2 text-[10px] font-bold text-white/35">
                                        Ï°??úÍ∏∞Í∞Ä ?ÜÏúºÎ©?AÏ°∞Î°ú ?∏Ïãù?©Îãà??
                                    </p>
                                 </div>
                                <textarea
                                    ...
                                    placeholder={"?? 1 Î¥âÏ?/?ÅÏú§ vs ?ÅÌò∏/Í¥ëÌòÑ 19:00"}
                                />
                            </div>
                            ...
                            <div className="overflow-visible rounded-[24px] border border-white/10 bg-[#111111] p-4">
                                ...
                                {manualPasteMatchCount === 0 ? (
                                    <div className="rounded-[18px] border border-dashed border-white/10 py-8 text-center text-[12px] font-bold text-white/35">
                                        ?ÄÏßÑÏùÑ Î∂ôÏó¨?£ÏúºÎ©?Í≤ΩÍ∏∞ ÎØ∏Î¶¨Î≥¥Í∏∞Í∞Ä ?úÏãú?©Îãà??
                                    </div>
                                ) : (
                                                ...
                                                <div className="flex items-center justify-between gap-3">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 text-[12px] font-black text-white">
                                                            <span className="truncate text-right">{row.teamA.filter(Boolean).join(' / ') || '?ïÏù∏ ?ÑÏöî'}</span>
                                                            <span className="rounded-full border border-[#C9B075]/35 px-2 py-0.5 text-[9px] font-black text-[#C9B075]">VS</span>
                                                            <span className="truncate">{row.teamB.filter(Boolean).join(' / ') || '?ïÏù∏ ?ÑÏöî'}</span>
                                                        </div>
                                                        {!row.isValid && (
                                ...
                                <div className="mt-4 border-t border-white/10 pt-4">
                                    ...
                                    {!canProceedToNameMatching && (
                                        <p className="mt-2 text-center text-[10px] font-bold text-white/35">
                                            ?ÄÏßÑÏùÑ 1Í∞??¥ÏÉÅ Î∂ôÏó¨?£ÏúºÎ©??¥Î¶Ñ Îß§Ïπ≠?ºÎ°ú ?¥Îèô?????àÏäµ?àÎã§.
                                        </p>
                                    )}
                                </div>
                            </div>
                            ...
                            <p className="px-1 text-[10px] font-bold leading-relaxed text-white/35">
                                ?§Ïùå ?®Í≥Ñ?êÏÑú ?¥Î¶Ñ??Î©§Î≤Ñ/Í≤åÏä§?∏Ï? Îß§Ïπ≠?©Îãà?? ?ÑÏßÅ ?Ä?•Ï? ?òÏ? ?äÏäµ?àÎã§.
                            </p>
                        </div>
                    )}
                            ...
                            <div className="overflow-visible rounded-[24px] border border-[#C9B075]/20 bg-[#141414] p-5">
                                <div className="mb-4 flex items-start justify-between gap-3">
                                    <div>
                                        ...
                                        <h2 className="mt-1 text-xl font-black text-white">?¥Î¶Ñ Îß§Ïπ≠</h2>
                                        <p className="mt-2 text-[11px] font-bold leading-relaxed text-white/45">
                                            Î∂ôÏó¨?£Ï? ?¥Î¶Ñ??Î©§Î≤Ñ ?êÎäî Í≤åÏä§?∏Ï? ?∞Í≤∞?©Îãà??
                                        </p>
                                    </div>
                                            ...
                                            >
                                                {match.candidates.map(candidate => (
                                                    <option key={candidate.id} value={candidate.id} className="bg-[#111111] text-white">
                                                        {getManualMemberName(candidate)} / Î©§Î≤Ñ
                                                    </option>
                                                ))}
                                                <option value="guest" className="bg-[#111111] text-white">
                                                    {match.originalName}(G) / Í≤åÏä§?∏Î°ú ?¨Ïö©
                                                </option>
                                            </select>
                                        </div>
                                    ))}
                    ...
                    {manualInputMode === 'PASTE' && manualStep === 'RULES' && (
                        <div className="rounded-[24px] border border-[#C9B075]/20 bg-[#141414] px-5 py-8 text-center">
                            <p className="text-lg font-black text-white">Î£??§Ï†ï?Ä ?§Ïùå ?®Í≥Ñ?êÏÑú ?∞Í≤∞?©Îãà??</p>
                            <p className="mt-3 text-[12px] font-bold leading-relaxed text-white/40">
                                ?¥Î≤à ?ëÏóÖ?êÏÑú???¥Î¶Ñ Îß§Ïπ≠Í≥?Îß§Ïπ≠ ?ÅÏö© ÎØ∏Î¶¨Î≥¥Í∏∞ÍπåÏ?Îß??Ä???ÜÏù¥ Ï§ÄÎπÑÌñà?µÎãà??
                            </p>
                            <button
                                type="button"
                                onClick={() => setManualStep('MATCH_NAMES')}
                    ...
                    {manualInputMode === 'DIRECT' && (
                        <div className="rounded-[30px] border border-white/10 bg-white/[0.04] px-5 py-8 text-center">
                            <p className="text-lg font-black text-white">ÏßÅÏ†ë ÎßåÎì§Í∏?Ï§ÄÎπ?Ï§?/p>
                            <p className="mt-3 text-[12px] font-bold leading-relaxed text-white/40">
                                Ï∂îÌõÑ Ï∞∏Í??êÎ? ?†ÌÉù?òÍ≥† Í≤ΩÍ∏∞Î≥??Ä??ÏßÅÏ†ë Íµ¨ÏÑ±?òÎäî Î∞©Ïãù?ºÎ°ú ?ïÏû•?©Îãà??
                            </p>
                        </div>
                    )}
                    ...
                    {manualInputMode === 'OCR' && (
                        <div className="rounded-[30px] border border-white/10 bg-white/[0.04] px-5 py-8 text-center">
                            <p className="text-lg font-black text-white">Ï∫°Ï≤ò ?∏Ïãù Coming Soon</p>
                            <p className="mt-3 text-[12px] font-bold leading-relaxed text-white/40">
                                ?¥Î?ÏßÄ/OCR Í∏∞Î∞ò ?ÄÏß??∏Ïãù?Ä ?àÏ†ï???¥ÌõÑ Î≥ÑÎèÑ ?®Í≥ÑÎ°?Í≤Ä?†Ìï©?àÎã§.
                            </p>
                        </div>
                    )}
                </section>
                ...
                <header className="grid grid-cols-3 px-6 mb-4 items-center h-12 shrink-0">
                    ...
                    <div className="text-center flex flex-col items-center gap-2">
                        <div className="flex flex-col items-center">
                            ...
                            <h1 className="text-3xl font-black italic tracking-tighter uppercase whitespace-nowrap text-white leading-none">Í≤ΩÍ∏∞ ?ÄÏß??§Ï†ï</h1>
                        </div>
                        {isAdmin && (
                    ...
                    <div className="flex justify-end">
                        ...
                        >
                            ...
                            <span className="text-[9px] font-black uppercase tracking-tighter">Ï¥àÍ∏∞??/span>
                        </button>
                    </div>
                </header>
                ...
                <div className="px-6 space-y-12 max-w-lg mx-auto w-full">
                    ...
                    <section className="space-y-4">
                        ...
                        <input
                            ...
                            placeholder="Ex: 2026-03-27 ?åÏó∞ ?ïÍ∏∞??
                        />
                    </section>
                        ...
                        <div className="mt-4 space-y-4">
                            ...
                            <div style={{ borderRadius: '18px', border: '1px solid rgba(201,176,117,0.18)', background: 'rgba(201,176,117,0.06)', padding: '12px 14px' }}>
                                <p className="text-[11px] font-bold leading-relaxed text-white/45">
                                    ?òÎèô Íµ¨ÏÑ±?Ä ?πÎ¶¨ ?êÏàò 6??Í≥†Ï†ï, 1:1 ?úÏûë??Í∏∞Î≥∏ ?ÑÏ†úÎ°??¨Ïö©?©Îãà??
                                </p>
                            </div>
                        </div>
                    </section>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4" style={{ marginTop: '20px' }}>
                                {(['RANDOM', 'AGE', 'AWARD', 'MBTI'] as const).map(mode => (
                                    ...
                                    >
                                        {mode === 'RANDOM' ? 'RANDOM' : mode === 'AGE' ? 'YB/OB' : mode === 'AWARD' ? '?ÖÏÉÅ/ÎπÑÏûÖ?? : 'MBTI'}
                                    </button>
                                ))}
                            </div>
                        </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginTop: '16px' }}>
                                {fixedPartners.map((pair, idx) => (
                                    <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#141414', padding: '20px 24px', borderRadius: '24px', border: '1px solid #2A2A2A' }}>
                                        ...
                                        <button onClick={() => setFixedPartners(prev => prev.filter((_, i) => i !== idx))} className="w-10 h-10 rounded-full bg-[#1C1C1C] flex items-center justify-center text-red-500/50 hover:bg-red-500/20 hover:text-red-500 transition-all text-2xl leading-none">√ó</button>
                                    </div>
                                ))}
                                <button
                    ...
                    <section style={{ background: '#1E1E1E', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '40px', padding: '28px 24px', marginTop: '12px', overflow: 'visible' }}>
                        ...
                        <div style={{ marginTop: '24px', paddingTop: '24px', borderTop: '1px solid #333' }}>
                            ...
                            <textarea
                                ...
                                placeholder="?†ÎÑàÎ®ºÌä∏ Í∑úÏπô???ÖÎ†•?òÏÑ∏??.."
                            />
                        </div>
                    </section>
                ...
                {isAdmin && (
                    <div style={{ position: 'fixed', bottom: '120px', left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: '450px', padding: '0 20px', zIndex: 9999, pointerEvents: 'none', boxSizing: 'border-box' }}>
                        <div style={{ width: '100%', margin: '0 auto', pointerEvents: 'auto' }}>
                            ...
                            >
                                {isManualRulesMode ? (isGenerating ? '?ùÏÑ± Ï§?..' : 'Î∂ôÏó¨?£Ï? ?ÄÏßÑÏúºÎ°??ùÏÑ±') : isGenerating ? 'GENERATE...' : 'ÏµúÏ¢Ö ?ÄÏß??êÎèô ?ùÏÑ±! ??'}
                            </button>
                        </div>
                    </div>
                )}
                    ...
                    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(12px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
                        <div style={{ background: '#1C1C1C', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '32px', width: '100%', maxWidth: '400px', padding: '32px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                                ...
                                <button onClick={() => setPartnerSelectSource(null)} style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'rgba(255,255,255,0.08)', border: 'none', color: 'rgba(255,255,255,0.6)', fontSize: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>√ó</button>
                            </div>
                            ...
                            <p style={{ textAlign: 'center', fontSize: '11px', color: 'rgba(255,255,255,0.3)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.15em', margin: 0 }}>
                                {partnerSelectSource === 'NEW' ? 'Ï≤?Î≤àÏß∏ ?åÎ†à?¥Ïñ¥Î•??†ÌÉù?òÏÑ∏?? : (allMembers.find(x => x.id === partnerSelectSource)?.nickname + (allMembers.find(x => x.id === partnerSelectSource)?.is_guest ? ' (G)' : '')) + '???åÌä∏?àÎ? ?†ÌÉù?òÏÑ∏??}
                            </p>
                        </div>
                    </div>
                )}
            ...
            <main className="flex flex-col min-h-screen bg-black text-white font-sans w-full relative overflow-hidden">
                    ...
                    {allActiveSessions.length === 0 ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-center px-12 opacity-20 group">
                            ...
                            <p className="text-[10px] font-medium leading-relaxed uppercase tracking-tighter max-w-xs">
                                ?ÑÏû¨ ÏßÑÌñâ Ï§ëÏù∏ Í≥µÍ∞ú ?∏ÏÖò???ÜÏäµ?àÎã§.<br />Í¥ÄÎ¶¨ÏûêÍ∞Ä Í≤ΩÍ∏∞Î•??ùÏÑ±?òÎ©¥ ?¨Í∏∞???úÏãú?©Îãà??
                            </p>
                        </div>
                    ) : (
                                ...
                                >
                                    ...
                                    <div className="flex items-start justify-between mb-8">
                                        <div className="flex flex-col gap-1">
                                            <span className={`text-[10px] font-black tracking-[0.3em] uppercase ${isLatest ? 'text-[#C9B075]' : 'text-white/40'}`}>
                                                {s.id.includes('KDK') ? '?ïÍ∏∞ KDK Îß§Ïπò' : '?§Ìéò??Îß§Ïπò'}
                                            </span>
                                    ...
                                    <div className="grid grid-cols-3 gap-4 border-t border-white/5 pt-6">
                                        <div className="flex flex-col">
                                            <span className="text-[8px] font-black text-white/20 uppercase tracking-widest mb-1 font-mono">Ï∞∏Í? ?∏Ïõê</span>
                                            <span className="text-lg font-black text-white italic">{s.playerCount}<span className="text-[10px] ml-0.5 opacity-30">Î™?/span></span>
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-[8px] font-black text-white/20 uppercase tracking-widest mb-1 font-mono">ÏßÑÌñâ Í≤ΩÍ∏∞</span>
                                        ...
                                        <div className="flex flex-col items-end">
                                            <span className="text-[10px] font-black text-[#C9B075] uppercase tracking-tighter mt-2">?ÖÏû•?òÍ∏∞ ?°Ô∏è</span>
                                        </div>
                                    </div>
                ...
                { (allActiveSessions.length === 0 || isAdmin) && (
                    <div className="absolute bottom-10 left-1/2 -translate-x-1/2 w-full px-8 pb-4">
                        ...
                        >
                            {isAdmin ? '+ ?àÎ°ú???Ä???ùÏÑ±?òÍ∏∞' : 'Ï§ÄÎπ???Í≤ΩÍ∏∞Í∞Ä ?ÜÏäµ?àÎã§'}
                        </button>
                    </div>
                )}
            </main>
        ...
        <main className="flex flex-col min-h-screen bg-gradient-to-br from-[#0a0a0b] via-[#121214] to-[#0a0a0b] text-white font-sans w-full relative pb-60" style={{ paddingBottom: 'calc(240px + env(safe-area-inset-bottom))' }}>
            ...
            <header className="px-6 py-4 flex items-center justify-between h-18 relative z-[200] bg-[#09090B] border-b border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.5)] backdrop-blur-xl">
                ...
                <div className="flex-1 flex items-center justify-end gap-3">
                    <div className="flex items-center gap-2">
                        <button onClick={execCopySchedule} className="w-8 h-8 bg-white/5 border border-white/10 rounded-lg flex items-center justify-center text-[#C9B075] hover:bg-[#C9B075]/20 active:scale-90 transition-all" title="Í≤∞Í≥º Î≥µÏÇ¨">?ìã</button>
                        <button onClick={copyFinalResults} className="w-8 h-8 bg-white/5 border border-white/10 rounded-lg flex items-center justify-center text-[#C9B075] hover:bg-[#C9B075]/20 active:scale-90 transition-all" title="Í≤∞Í≥º Î≥¥Í≥†">?èÜ</button>
                    </div>
                    ...
                    <div className="flex flex-col items-end pr-1">
                        <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[8px] font-black text-white/40 uppercase tracking-tighter">
                                {syncStatus === 'HEALTHY' ? '?∞Í≤∞?? : syncStatus === 'IDLE' ? '?ÄÍ∏∞Ï§ë' : '?ôÍ∏∞??Ï§?}
                            </span>
                        </div>
                ...
                <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <span className="text-[9px] font-bold text-white/60 tracking-tighter italic uppercase truncate">
                            {matchRules?.slice(0, 30) || '1:1 ?úÏûë, ?∏Ïóê?? ?Ä??3:3'}
                        </span>
                        {adminModeManual && isAdmin && (
                             <button onClick={() => setShowMemberEditModal(true)} className="flex items-center justify-center w-5 h-5 bg-white/5 rounded-full text-[#C9B075]/40 hover:text-[#C9B075] text-[10px] transition-all active:scale-90">?ôÔ∏è</button>
                        )}
                    </div>
                </div>
            </div>
                                        ...
                                        <div key={group} className="space-y-3">
                                            <div className="flex flex-col" style={{ marginBottom: '16px', marginTop: '32px' }}>
                                                <h3 className="text-lg font-black italic tracking-tighter uppercase text-white ml-2" style={{ filter: 'drop-shadow(0 2px 4px rgba(255,255,255,0.2))' }}>
                                                    {isManualLiveSession ? `${group}Ï°?WAITING` : `${isB ? 'BLUE' : 'GOLD'} WAITING`}
                                                </h3>
                                                                ...
                                                                {matchesInRound.map((m, idx) => {
                                                                    ...
                                                                    return (
                                                                        ...
                                                                        <WaitingMatchCard 
                                                                            ...
                                                                            onStart={(id) => {
                                                                                if (!isAdmin || !adminModeManual) return triggerAccessDenied("Í≤ΩÍ∏∞ ?¨ÏûÖ?Ä Í¥ÄÎ¶¨Ïûê Î™®Îìú?êÏÑúÎß?Í∞Ä?•Ìï©?àÎã§.");
                                                                                startMatch(id);
                                                                            }}
                                                                        />
                                        ...
                                        .map((m, idx) => {
                                            ...
                                            return (
                                                <CompletedMatchCard 
                                                    ...
                                                    isAdmin={isAdmin}
                                                    onResetStatus={(id) => {
                                                        if (window.confirm("??Í≤ΩÍ∏∞Î•??§Ïãú 'ÏßÑÌñâ Ï§?ONGOING)' ?ÅÌÉúÎ°??òÎèåÎ¶¨ÏãúÍ≤†Ïäµ?àÍπå?\n(?êÏàòÍ∞Ä 1:1Î°?Ï¥àÍ∏∞?îÎêòÎ©???Çπ?êÏÑú ?ºÏãú ?úÏô∏?©Îãà??")) {
            ...
            {allMatchesScored && step === 3 && activeTab === 'MATCHES' && (
                <div className="fixed bottom-[200px] left-1/2 -translate-x-1/2 w-full max-w-[450px] px-6 z-[60] animate-in slide-in-from-bottom-10 fade-in duration-500">
                    ...
                    >
                        <span>?èÜ Ï¶âÏãú ?úÏúÑ Î∞?Ï∂ïÌïò ?îÎ©¥ Î≥¥Îü¨Í∞ÄÍ∏?/span>
                    </button>
                </div>
            )}
            ...
            {showRankingModal && (
                <div className="fixed inset-0 z-[100] flex flex-col bg-black/98 backdrop-blur-2xl">
                    <header className="p-6 border-b border-white/5 flex items-center justify-between bg-[#14141F]">
                        ...
                        <button onClick={() => setShowRankingModal(false)} className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center text-xl text-white">√ó</button>
                    </header>
                ...
                <div className="fixed inset-0 z-[300] bg-black/60 backdrop-blur-2xl flex flex-col p-6 overflow-hidden">
                    <header className="flex items-center justify-between mb-8 border-b border-white/10 pb-6">
                        <div className="flex flex-col">
                            ...
                            <h2 className="text-2xl font-black italic text-white uppercase tracking-tighter">Ï∞∏ÏÑù???òÏãú ?òÏ†ï</h2>
                        </div>
                        <button onClick={() => setShowMemberEditModal(false)} className="w-12 h-12 bg-white/5 rounded-full flex items-center justify-center text-white/20 text-3xl hover:bg-white/10 transition-colors">√ó</button>
                    </header>
                    <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-8">
                                ...
                                {[...allMembers, ...tempGuests].map(m => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            onClick={() => {
                                                if (isBusy) {
                                                    alert("?ÑÏû¨ Í≤ΩÍ∏∞ Ï§ëÏù∏ ?†Ïàò??Í∏∞Í∂å Ï≤òÎ¶¨?????ÜÏäµ?àÎã§. Í≤ΩÍ∏∞Í∞Ä ?ùÎÇú ??Ï°∞Ï†ï??Ï£ºÏÑ∏??");
                                                    return;
                                                }
                                                toggleMember(m.id);
                                            }}
                        ...
                        <p className="text-[10px] font-bold text-white/30 leading-relaxed uppercase tracking-widest text-center italic">
                            ?í° ?†ÏàòÎ•?Ï∂îÍ??òÍ±∞???úÍ±∞?òÎ©¥, <br />?ÑÏßÅ ?¨ÏûÖ?òÏ? ?äÏ? Î™®Îì† ?êÎèô ?ÄÏßÑÏù¥ ?§Ïãú ?ùÏÑ±?©Îãà??
                        </p>
                    </div>
                    <div className="mt-8">
                        <button
                        ...
                        >
                            {isGenerating ? '?ÄÏß??¨Íµ¨??Ï§?..' : '?íæ ?§ÏãúÍ∞??∏Ïõê Î≥ÄÍ≤ΩÏÇ¨???ÅÏö©'}
                        </button>
                    </div>
                </div>
            )}
            ...
            {showResetConfirm && (
                <CustomConfirmModal
                    title="?ÄÏßÑÌëú Ï¥àÍ∏∞??
                    message="?ÑÏû¨ ÏßÑÌñâ Ï§ëÏù∏ Î™®Îì† ?ÄÏßÑÍ≥º ?†ÌÉù??Î©§Î≤Ñ ?ïÎ≥¥Î•???†ú?òÍ≥† Ï≤òÏùåÎ∂Ä???§Ïãú ?úÏûë?òÏãúÍ≤†Ïäµ?àÍπå?"
                ...
                <div className="fixed inset-0 z-[5000] bg-black flex flex-col items-center justify-center animate-in fade-in duration-1000">
                    ...
                    <div className="relative z-10 flex flex-col items-center text-center px-12 space-y-8">
                        <div className="w-40 h-40 rounded-full bg-[#C9B075]/10 border-2 border-[#C9B075]/50 flex items-center justify-center animate-bounce shadow-[0_0_80px_rgba(201,176,117,0.5)]">
                            <span className="text-8xl drop-shadow-2xl">?èÜ</span>
                        </div>
                        <div className="space-y-4">
