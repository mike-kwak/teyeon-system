...
//   Client Component 인 page 에서는 Route Segment Config(dynamic/revalidate)를 export 하지 않는다.
import React from 'react';
import { useParams } from 'next/navigation';
import { fetchPublicNoticeByToken, type PublicNoticeView } from '@/lib/finance/noticesService';
import PublicFinanceNotice from '@/components/finance/PublicFinanceNotice';

/**
 * /finance/public/[token] — 회원 공지용 미납 현황 공개 페이지(로그인 불필요).
 *   token 으로 get_public_finance_notice RPC 조회. 비활성/없음이면 안내만 노출.
 *   BottomNav 는 /finance/public 경로에서 숨김(공개 chrome 없음).
 */
export default function PublicFinanceNoticePage() {
    const params = useParams<{ token: string }>();
    const token = typeof params?.token === 'string' ? params.token : Array.isArray(params?.token) ? params!.token[0] : '';

    const [state, setState] = React.useState<'loading' | 'ready' | 'unavailable'>('loading');
    const [notice, setNotice] = React.useState<PublicNoticeView | null>(null);

    React.useEffect(() => {
        let cancelled = false;
        (async () => {
            if (!token) { setState('unavailable'); return; }
            const data = await fetchPublicNoticeByToken(token);
            if (cancelled) return;
    ...
    return <PublicFinanceNotice notice={notice} />;
}
