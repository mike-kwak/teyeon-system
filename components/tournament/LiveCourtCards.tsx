...
export const PlayingMatchCard = ({ 
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-1 flex-col justify-center p-4 py-7 sm:py-8">
                <div className="grid flex-grow grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2.5 sm:gap-3">

                    {/* TEAM A BLOCK */}
                    <div className="relative flex h-[70px] w-full min-w-0 flex-col items-center justify-center overflow-hidden rounded-[18px] border border-white/5 bg-white/5 transition-all duration-300 sm:h-[76px]">
                        <div className="flex flex-col items-center justify-center w-full px-1 sm:px-2 gap-1 min-w-0">
                            <div className="text-white font-black leading-none relative z-0 truncate w-full flex items-center justify-center gap-0.5" style={{ filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.8))' }}>
                                <span className="truncate text-[clamp(13px,4vw,16px)]">{getPlayerName(match.playerIds[0], match.id).replace(/\s*\(G\)$/i, '')}</span>
                            ...
                            <div className="text-white font-black leading-none relative z-0 truncate w-full flex items-center justify-center gap-0.5" style={{ filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.8))' }}>
                                <span className="truncate text-[clamp(13px,4vw,16px)]">{getPlayerName(match.playerIds[1], match.id).replace(/\s*\(G\)$/i, '')}</span>
                    ...
                    <div className="relative flex h-[70px] w-full min-w-0 flex-col items-center justify-center overflow-hidden rounded-[18px] border border-white/5 bg-white/5 transition-all duration-300 sm:h-[76px]">
                        <div className="flex flex-col items-center justify-center w-full px-1 sm:px-2 gap-1 min-w-0">
                            <div className="text-white font-black leading-none relative z-0 truncate w-full flex items-center justify-center gap-0.5" style={{ filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.8))' }}>
                                <span className="truncate text-[clamp(13px,4vw,16px)]">{getPlayerName(match.playerIds[2], match.id).replace(/\s*\(G\)$/i, '')}</span>
                            ...
                            <div className="text-white font-black leading-none relative z-0 truncate w-full flex items-center justify-center gap-0.5" style={{ filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.8))' }}>
                                <span className="truncate text-[clamp(13px,4vw,16px)]">{getPlayerName(match.playerIds[3], match.id).replace(/\s*\(G\)$/i, '')}</span>
                ...
                <button
                    ...
                    className="mt-4 flex h-12 w-full shrink-0 items-center justify-center rounded-[16px] border border-[#8E7A4A]/40 bg-transparent transition-all hover:bg-[#8E7A4A]/25 active:scale-95 sm:h-14"
...
export const WaitingMatchCard = ({ 
    ...
    return (
        <div key={match.id} className="active:scale-98 relative grid grid-cols-[45px_minmax(0,1fr)_78px] items-center overflow-hidden rounded-2xl px-3 py-[22px] transition-all sm:grid-cols-[48px_minmax(0,1fr)_84px] sm:px-4 sm:py-6" style={{ transform: 'none', background: 'rgba(255, 255, 255, 0.05)', backdropFilter: 'blur(64px)', border: 'none', borderTop: `2px solid ${isGroupB ? 'rgba(0, 229, 255, 0.3)' : 'rgba(255, 255, 255, 0.3)'}`, boxShadow: `0 20px 50px rgba(0,0,0,0.9), 0 0 15px ${isGroupB ? 'rgba(0, 229, 255, 0.1)' : 'rgba(201, 176, 117, 0.03)'}`, filter: `drop-shadow(0 0 10px ${col}33)` }}>
            <div className="flex items-center justify-center">
                <div className="flex h-10 w-10 shrink-0 flex-col items-center justify-center rounded-full border border-white/20 text-black shadow-[0_0_10px_rgba(0,0,0,0.2)] sm:h-11 sm:w-11" style={{ background: `linear-gradient(135deg, ${col}, ${col}aa)` }}>
                    <span className="text-[8px] font-black leading-none opacity-40">R{match.round}</span>
                    <span className="text-[14px] font-[1000] leading-none uppercase sm:text-[15px]">{isGroupB ? 'B' : 'G'}{Math.max(1, displayIndex)}</span>
                </div>
            </div>

            <div className="flex min-w-0 items-center justify-center gap-2 px-1 text-center sm:gap-2.5" style={{ filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.8))' }}>
                <span className="flex min-w-0 flex-1 items-center justify-center gap-0.5 truncate text-[15px] font-bold leading-none text-white sm:text-[17px]">
                ...
                <span className="text-[10px] font-black uppercase italic tracking-tighter opacity-25 shrink-0" style={{ color: col }}>vs</span>
                <span className="flex min-w-0 flex-1 items-center justify-center gap-0.5 truncate text-[15px] font-bold leading-none text-white sm:text-[17px]">
            ...
            <div className="flex items-center justify-end">
                <button
                    ...
                    className={`rounded-xl px-4 py-3.5 text-[12px] font-black uppercase shadow-xl transition-all active:scale-95 sm:px-5 sm:py-4 sm:text-[13px] ${(isStartingMatch || hasConflict) ? 'bg-zinc-800 text-white/5 cursor-not-allowed opacity-50' : '!text-black hover:opacity-90'}`}
