-- KDK 공식 라이브 순위 RPC — 동률 규칙 개정: 연소자 우위 → 연장자 우위 (2026-08-05)
--   ⚠️ 사용자 승인 후 실행. add_kdk_live_official_ranking.sql 적용 이후에 실행한다.
--   롤백: update_kdk_official_ranking_oldest_first_rollback.sql
--
--   변경 내용: ORDER BY 의 birth_year 방향만 DESC → ASC.
--     그 외 일체 변경 없음 —
--       · 승수/득점(pf)/실점(pa)/득실(pf-pa) 집계식 동일
--       · 경기 포함 규칙(status='complete' AND score1<>score2) 동일
--       · 이름 해석/게스트 판정/(G) 접미 규칙 동일
--       · 반환 구조(name, wins, losses, pf, pa, diff, rank, isGuest) 동일
--       · birth_year/UUID/개인정보 미반환 정책 동일
--       · SECURITY DEFINER / SET search_path / REVOKE / GRANT 동일
--
--   공식 comparator (lib/kdk/officialRanking.ts 와 반드시 동일 — fixture 대조 필수):
--     ① 승수 ↓ ② 득실 ↓ ③ 출생연도 제공자 우선 → 제공 시 작은 값(연장자) 우선
--     ④ 이름(코드포인트 = COLLATE "C") ↑ ⑤ 내부 stable id ↑ (정렬에만 사용, 미반환)
--   NULL 정책: 출생연도 미제공자는 제공자보다 항상 후순위(방향 변경과 무관하게 기존과 동일).
--   birthYear: 게스트 = kdk_session_attendee_meta snapshot / 회원 = members."나이",
--   양쪽 모두 1900~현재연도만 인정(normalizeBirthYear 와 동일).
--
--   적용 범위: 이 함수는 '지금 계산하는' 라이브 순위만 만든다.
--   과거 공식 Archive 의 저장된 ranking_data 는 이 함수를 거치지 않으므로 영향 없음.
  ...
  SELECT display_name, wins, losses, pf, pa, (pf - pa) AS diff, is_guest,
         (ROW_NUMBER() OVER (
           ORDER BY wins DESC,
                    ...
                    birth_year ASC,             -- 제공 시 연장자(작은 연도) 우선
...
-- 실행 권한: anon(공개 전광판) + authenticated 만. 그 외 전부 revoke. (기존과 동일 — 확대 없음)
