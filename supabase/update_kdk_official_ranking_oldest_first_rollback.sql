-- [ROLLBACK] KDK 공식 라이브 순위 RPC — 연장자 우위 → 연소자 우위(개정 이전 상태)로 복구
--   ⚠️ 사용자 승인 후 실행. update_kdk_official_ranking_oldest_first.sql 을 되돌릴 때만 사용한다.
--   결과: add_kdk_live_official_ranking.sql 과 완전히 동일한 함수 정의로 복구된다.
--
--   변경 내용: ORDER BY 의 birth_year 방향만 ASC → DESC (그 외 일체 동일).
--     그 외 일체 변경 없음 —
