...
union all select 14, '[PII] standings RPC 가 접수 원장을 참조하지 않음',
        (select s_stand not like '%hosted_tournament_registrations%' from src)
...
union all select 41, '[fn] standings 본문에 쓰기 구문 없음',
        (select s_stand !~* '(^|[^a-z_])(insert|update|delete|truncate|alter|drop)[[:space:]]+' from src)
...
union all select 50, '[rank] 정렬 키가 승률 → 득실 두 단계로 고정',
        (select s_stand like '%order by c.win_rate desc nulls last, c.game_diff desc%' from src)
...
union all select 52, '[rank] 승률은 numeric 나눗셈(float 오차 배제)',
        (select s_stand like '%wins::numeric / nullif(a.played, 0)%' from src)
...
union all select 54, '[calc] COMPLETED 경기만 집계에 포함',
        (select s_stand like '%where status = ''completed''%' from src)
union all select 55, '[calc] placement 는 standings 집계에서 제외',
        (select s_stand like '%g.group_type = ''preliminary''%' from src)
union all select 56, '[calc] 조 기대 경기수를 N(N-1)/2 일반식으로 계산(3 하드코딩 없음)',
        (select s_stand like '%(gs.members * (gs.members - 1)) / 2%' from src)
union all select 57, '[state] rankingStatus 3종 모두 존재',
        (select s_stand like '%PROVISIONAL%' and s_stand like '%AGE_CHECK_REQUIRED%'
            and s_stand like '%''FINAL''%' from src)
union all select 58, '[state] CANCELLED 잔존 시 policyRequired 발급',
        (select s_stand like '%cancelled_matches_present%' from src)
union all select 59, '[state] 미완료 조는 무조건 PROVISIONAL',
        (select s_stand like '%when not gs.complete%then ''PROVISIONAL''%' from src)
union all select 60, '[qual] 미완료 조의 진출 판정은 전부 PENDING',
        (select s_stand like '%when not gs.complete then ''PENDING''%' from src)
union all select 61, '[qual] 동률 묶음이 진출권 안쪽이면 QUALIFIED',
        (select s_stand like '%rk.auto_rank + ts.tsize - 1 <= v_qualify then ''QUALIFIED''%' from src)
union all select 62, '[qual] 동률 묶음이 진출권 밖이면 NOT_QUALIFIED',
        (select s_stand like '%when rk.auto_rank > v_qualify then ''NOT_QUALIFIED''%' from src)
union all select 63, '[qual] 진출 인원이 한 곳(v_qualify)에만 정의됨',
        (select s_stand like '%v_qualify integer := 2%' from src)
union all select 64, '[rank] tie group 은 전원 확정됐을 때만 resolved',
        (select s_stand like '%= t.tsize as resolved%' from src)
...
union all select 81, '[lock] standings 는 lock 을 잡지 않음(조회 전용)',
        (select s_stand not like '%pg_advisory%' from src)
